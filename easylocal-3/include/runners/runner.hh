...
public:
  ...
  unsigned long int iteration;
    std::vector<std::pair<std::chrono::high_resolution_clock::time_point,int>> every_20000;
protected:
...
CostStructure Runner<Input, Solution, CostStructure>::Go(Solution&s)
{
  InitializeRun(s);
    ...
    auto start_inner = last_update;
    std::cout << std::chrono::duration_cast<std::chrono::seconds>(start_inner - last_update).count() << ", " << best_state_cost.total << std::endl;
  while (!MaxEvaluationsExpired() && !StopCriterion() && !LowerBoundReached() && !this->TimeoutExpired())
  {
    ...
    CompleteIteration();
    ;
    if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::high_resolution_clock::now() - last_update).count() >= 10.0)
    {
        std::cout << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::high_resolution_clock::now() - start_inner).count() << ", " << best_state_cost.total << std::endl;
...
{
    return evaluations >= max_evaluations; // max_evaluations.IsSet() && evaluations >= max_evaluations;
}
